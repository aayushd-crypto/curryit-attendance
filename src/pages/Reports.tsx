...
export default function ReportsPage() {
  ...
  const [location, setLocation]   = useState<'all' | 'office' | 'cmk'>(
    role === 'cmk_coordinator' ? 'cmk' : 'all'
  )
  ...
  const loadReport = async () => {
    ...
    setRows(filtered.map((r: any) => ({
      ...
      location:      statusLabel(r.location ?? ''),
    ...
    <div className="max-w-6xl mx-auto space-y-5">
      <div className="page-header flex-wrap gap-2">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => exportToPDF(
              `Attendance Report — ${formatDate(dateFrom)} to ${formatDate(dateTo)}`,
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[500px]">
              ...
              <tbody>
                {rows.length === 0 ? (
                  <tr><td colSpan={9} className="text-center py-12 text-gray-400">No records found for selected filters.</td></tr>
                ) : (
                  rows.map((r, i) => (
                    <tr key={i}>
                      ...
                      <td>
                        <span className={`px-2 py-0.5 rounded-full text-xs ${r.location === 'CMK' ? 'bg-amber-50 text-amber-700' : 'bg-blue-50 text-blue-700'}`}>
                          {r.location}
                        </span>
                      </td>
                      <td>
                        {r.status === 'Present' ? <span className="badge-present">Present</span>
                          : r.status === 'Absent' ? <span className="badge-absent">Absent</span>
                          : r.status === 'On Leave' ? <span className="badge-leave">On Leave</span>
                          : <span className="badge-pending">{r.status}</span>}
                      </td>
                      <td className="text-gray-500 capitalize">{r.work_mode}</td>
